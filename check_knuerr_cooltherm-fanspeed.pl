...
sub checkStati {
	...
	$stateFan = 'WARNING' if $result{$fanspeed} / 10 < 20;
	$stateFan = 'WARNING' if $result{$fanspeed} / 10 > 80;
	$stateFan = 'CRITICAL' if $result{$fanspeed} / 10 < 10;
	$stateFan = 'CRITICAL' if $result{$fanspeed} / 10 > 90;

	raiseGlobalState($stateFan);
	printf(
		'%s Lüfterdrehzahl: %s %% | Luefterdrehzahl=%.1f%%;20:80;10:90;0;100',
		$stateFan,
        $result{$fanspeed} / 10,
        $result{$fanspeed} / 10
	)
