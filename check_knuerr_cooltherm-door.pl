...
sub checkStati {
        my $frontdoor = $baseOid . '.9.1.17.0';
        my $reardoor = $baseOid . '.9.1.18.0';
	...
	$stateDoor = 'WARNING' if $result{$frontdoor} == 1;
	$stateDoor = 'CRITICAL' if $result{$reardoor} == 1;

	raiseGlobalState($stateDoor);

	printf(
		'%s Türstatus - Vordertüre: %s, Hintertüre: %s',
		$stateDoor,
        $st{$result{$frontdoor}},
        $st{$result{$reardoor}}
	)
