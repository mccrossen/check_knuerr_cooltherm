...
sub checkHumidity {
	...
	my $state = 'OK';
	my $lowWarn  = $result{$lowCrit} / 10;
	my $highWarn = $result{$highCrit} /10;
	if ($result{$highCrit} - $result{$lowCrit} > 40) {
		if ($result{$current} / 10 <= $result{$lowCrit} / 10 + 10
		    || $result{$current} / 10 >= $result{$highCrit} / 10 - 10) {
			$state = 'WARNING';
		}
		$lowWarn  = $result{$lowCrit} / 10 + 10;
		$highWarn = $result{$highCrit} / 10 - 10;
	}


	if ($result{$current} / 10 <= $result{$lowCrit} / 10
        || $result{$current} / 10 >= $result{$highCrit} / 10) {
		$state = 'CRITICAL';
	}
	...
	printf('%s | humidity=%s', join(', ', @info), join(' ', $performance{'humidity'}));
}
