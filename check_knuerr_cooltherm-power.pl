...
sub checkStati {
        my $powerAalert = $baseOid . '.9.1.7.0';
        my $powerBalert = $baseOid . '.9.1.8.0';
	
        my @oids    = (
		$powerAalert, $powerBalert
	);
	my %st = (
		0 => 'OK',
		1 => 'NO'
	...
	my $failedPower = $result{$powerAalert} + $result{$powerBalert};
    	$statePower = 'WARNING' if $failedPower > 0;
    	$statePower = 'CRITICAL' if $failedPower > 1;

	raiseGlobalState($statePower);

	printf(
		'%s Stromstatus - Netz-A: %s, Netz-B: %s',
		$statePower,
        $st{$result{$powerAalert}},
        $st{$result{$powerBalert}}
	)
