...
sub checkStati {
    my $valueposition   = $baseOid . '.1.11.3.0';
	...
	my $stateValueposition = 'OK';

	$stateValueposition = 'WARNING' if $result{$valueposition} / 10 < 30;
	$stateValueposition = 'CRITICAL' if $result{$valueposition} / 10 < 10;

	raiseGlobalState($stateValueposition);
	printf(
		'%s Ventilposition: %s %% | Ventilposition=%.1f%%;30:100;10:100;0;100',
		$stateValueposition,
        $result{$valueposition} / 10,
        $result{$valueposition} / 10
	)
}
